...
static const RegisterMap registerMap[] = {
    ...
    {"ep", REG_EP},   {"cp", REG_CP},   {"none", REG_UNKNOWN}};
...
void Push(Machine *machine, Data value) {
  if (machine->stackSize >= STACK_CAPACITY) {
    fprintf(stderr,
            "Stack overflow when trying to push value to stack. Aborted.\n");
    exit(1);
  }
...
int Pop(Machine *machine) {
  if (machine->stackSize <= 0) {
    fprintf(stderr,
            "Stack underflow when trying to pop from stack. Aborted.\n");
    exit(1);
  }
...
void RunInstructions(Machine *machine) {
  ...
  case OP_PUSH:
    if (inst.data.value.type == TY_STR &&
      ...
      Push(machine,
           machine
               ->memory[GetRegisterFromName((char *)inst.data.value.data.ptr)]);
      break;
    }
