...
static const OpcodeEntry opcodeTable[] = {
    ...
    {"jne", OP_JNE},    {"je", OP_JE},        {"jg", OP_JG},
    {"jge", OP_JGE},    {"jl", OP_JL},        {"jle", OP_JLE},
...
void TypeError(Lexer *lexer, char *optMsg) {
  char *line = GetLine(lexer);
  fprintf(stderr, "%s:%ld:%ld: type error", lexer->filePath, lexer->lineNumber,
          lexer->charIndex);
...
void ToOperandType(Operand *operands, int index, char *operand) {
  ...
  if (operand[0] == LXR_STR_CHAR &&
      operand[strlen(operand) - 1] == LXR_STR_CHAR) {
  ...
  case 2:
    if (operation == OP_MOV) {
      if (operands[0].type == TY_STR &&
        ...
        snprintf(t.text, textLen, "%s %s, %s", keyword,
    ...
    i.data.registers.dest = operands[1].data.i64;
    snprintf(t.text, textLen, "%s %s, %s", keyword,
             GetRegisterName(i.data.registers.src),
             GetRegisterName(i.data.registers.dest));
    break;
  case 1:
    ...
    if (operands[0].type == TY_STR)
      snprintf(t.text, textLen, "%s \"%s\"", keyword,
...
char *ParseOperand(Lexer *lexer, Opcode opcode) {
  ...
  if (lexer->text[lexer->charIndex] == LXR_LABEL_START &&
      (opcode == OP_JMP || opcode == OP_JE || opcode == OP_JG ||
       opcode == OP_JGE || opcode == OP_JL || opcode == OP_JLE ||
       opcode == OP_JNE)) {
    lexer->charIndex++;
  ...
  if (((opcode == OP_PUSH || opcode == OP_POP) &&
...
void ParseOperands(Lexer *lexer, Opcode opcode, Operand *operands) {
  if (opcode == OP_POP || opcode == OP_PUSH) {
    ...
    if (opcode == OP_PUSH &&
        (isdigit(operand[0]) || operand[0] == LXR_STR_CHAR)) {
  ...
  for (int opIndex = 0; opIndex < OperandsExpected(opcode); opIndex++) {
    ...
    if (opcode == OP_JMP || opcode == OP_JE || opcode == OP_JG ||
        opcode == OP_JGE || opcode == OP_JL || opcode == OP_JLE ||
        opcode == OP_JNE) {
...
Lexer ParseTokens(char *path) {
  ...
  while (lexer.charIndex <= lexer.textLength &&
         lexer.numTokens <= MAX_PROGRAM_SIZE) {
    ...
    if (lexer.charIndex < lexer.textLength &&
      ...
      if (lexer.text[lexer.charIndex] != LXR_STR_CHAR) {
        char buff[25] = {0};
        snprintf(buff, 25, "unknown character '%c'",
                 lexer.text[lexer.charIndex]);
        SyntaxError(&lexer, buff);
      }
    }
